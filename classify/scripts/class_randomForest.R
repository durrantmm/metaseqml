...
for (f in features){
  ...
  data_in <- read_tsv(f, n_max=n_max) %>% select(-Seq, -Kingdom, -Phylum, -Order, -Family, -Genus, -Species)
...
classif.task = makeClassifTask(data = data.frame(train_dataset), target = "Class")

mod = train(classif.lrn, classif.task)

out_rds <- file.path(output_dir, 'class.randomForest.rds')
saveRDS(mod, out_rds)
