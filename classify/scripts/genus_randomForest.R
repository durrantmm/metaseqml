...
for (f in features){
  ...
  data_in <- read_tsv(f, n_max=n_max) %>% select(-Seq, -Kingdom, -Phylum, -Class, -Order, -Family, -Species)
...
classif.task = makeClassifTask(data = data.frame(train_dataset), target = "Genus")

mod = train(classif.lrn, classif.task)

out_rds <- file.path(output_dir, 'genus.randomForest.rds')
saveRDS(mod, out_rds)
