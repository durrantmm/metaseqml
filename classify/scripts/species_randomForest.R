...
for (f in features){
  ...
  data_in <- read_tsv(f, n_max=n_max) %>% select(-Seq, -Kingdom, -Phylum, -Class, -Order, -Family, -Genus)
...
classif.task = makeClassifTask(data = data.frame(train_dataset), target = "Species")

mod = train(classif.lrn, classif.task)

out_rds <- file.path(output_dir, 'species.randomForest.rds')
saveRDS(mod, out_rds)
